...
async function downloadDDNSGo() {
  ...
  try {
    const response = await API.request(
      `{{.ServerUrl}}/@adminapi/admin/get_ThirdParty_ddnsgo?DownLoadlink=${encodeURIComponent(DownLoadlink)}&Version=${encodeURIComponent(Version)}&DdnsGOBinPath=${encodeURIComponent(DdnsGOBinPath)}&GitHubDownloadMirror=${encodeURIComponent(ThirdPartyExtGitHubDownloadMirror)}`,
      {},
      { needToken: true }
    )
...
async function resetDDNSGOPassword() {
  ...
  try {
    const res = await window.API.request(
      `{{.ServerUrl}}/@adminapi/admin/SpecialOPT?opt=ddnsgo_reset_admin&binPath=${encodeURIComponent(binPath)}&configPath=${encodeURIComponent(configPath)}`,
      {},
