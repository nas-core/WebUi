...
async function reloadCaddy() {
  ...
  try {
    ...
    const res = await window.API.request(
      `{{.ServerUrl}}/@adminapi/admin/SpecialOPT?opt=caddyreload&binPath=${encodeURIComponent(binPath)}&configPath=${encodeURIComponent(configPath)}`,
      {},
...
async function getServerFileContent(filePath, asText = true) {
  try {
    // 从后端读取文件内容
    const res = await window.API.request(`{{.ServerUrl}}/@adminapi/admin/getServerFile?path=${encodeURIComponent(filePath)}`, {}, { needToken: true })
...
async function saveServerFileContent(filePath, content) {
  try {
    const res = await window.API.request(
      `{{.ServerUrl}}/@adminapi/admin/saveServerFile?path=${encodeURIComponent(filePath)}`,
      {
        content: content,
      },
      { needToken: true }
