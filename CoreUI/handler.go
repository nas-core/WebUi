...
func Webui_handler(w http.ResponseWriter, r *http.Request, nsCfg *system_config.SysCfg, logger *zap.SugaredLogger, qpsCounter *uint64) {
	...
	if strings.HasSuffix(filePath, ".shtml") {
		...
		parsedContent = webuiLib.ReplaceTemplatePlaceholders(parsedContent, nsCfg.WebUICdnPrefix, "")
		if !isMinifyEnabled {
		...
		if isMinifyEnabled && isMinifiable(fileExt) { // 仅对可压缩文件类型执行压缩
			content, err = webuiLib.Exe_minify([]byte(webuiLib.ReplaceTemplatePlaceholders(string(content), nsCfg.Server.WebUIPrefix, "")), filePath, fileExt)
			if err != nil {
...
func handleFileContent(content []byte, fileExt string, nsCfg *system_config.SysCfg, w http.ResponseWriter) {
	...
	if isTextFile(fileExt) {
		content = []byte(webuiLib.ReplaceTemplatePlaceholders(string(content), nsCfg.Server.WebUIPrefix, ""))
	}
