...
async function handleApiRequest(url) {
  ...
  try {
    if (url.pathname === '/api/search') {
      ...
      try {
        console.log('DEBUG: Fetching proxied apiUrl:', PROXY_URL + encodeURIComponent(apiUrl)) // 调试信息
    ...
    if (url.pathname === '/api/detail') {
      ...
      try {
        console.log('DEBUG: Fetching proxied detailUrl:', PROXY_URL + encodeURIComponent(detailUrl)) // 调试信息
...
async function handleAggregatedSearch(searchQuery) {
  ...
  const searchPromises = availableSources.map(async (source) => {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`${source}源搜索超时`)), 8000))
      ...
      console.warn(`${source}源搜索失败2:`, error)
...
async function handleMultipleCustomSearch(searchQuery, customApiUrls) {
  ...
  const searchPromises = apiUrls.map(async (apiUrl, index) => {
    try {
      ...
      // 使用Promise.race添加超时处理
      const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`自定义API ${index + 1} 搜索超时`)), 8000))
      ...
      console.warn(`自定义API ${index + 1} 搜索失败1:`, error)
      return [] // 返回空数组表示该源搜索失败
    }
  })
...
;(function () {
  ...
  window.fetch = async function (input, init) {
    ...
    const requestUrl = typeof input === 'string' ? new URL(input, window.location.origin + window.location.pathname) : input.url
