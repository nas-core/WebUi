...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!localStorage.getItem('hasInitializedDefaults')) {
    // 默认选中资源
    selectedAPIs = DefaultSelectedAPISite
...
function resetSearchArea() {
  ...
  // 重置URL为主页
  try {
    window.history.pushState({}, `NasCoreTV - 免费在线视频搜索与观看平台`, `/`)
    // 更新页面标题
    document.title = `NasCoreTV - 免费在线视频搜索与观看平台`
  } catch (e) {
...
async function search() {
  ...
  try {
    ...
    try {
      ...
      window.history.pushState({ search: query }, `搜索: ${query} - NasCoreTV`, `./?s=${encodedQuery}`)
      // 更新页面标题
      document.title = `搜索: ${query} - NasCoreTV`
    } catch (e) {
      console.error('更新浏览器历史失败:', e)
      // 如果更新URL失败，继续执行搜索
    }
...
async function showDetails(id, vod_name, sourceCode) {
  ...
  try {
    ...
    // 显示来源信息
    const sourceName =
      data.videoInfo && data.videoInfo.source_name ? ` <span class="text-sm font-normal text-gray-400">(${data.videoInfo.source_name})</span>` : ''
...
async function importConfigFromUrl() {
  ...
  document.getElementById('confirmUrlImport').addEventListener('click', async () => {
    ...
    try {
      ...
      if (config.name !== 'NasCoreTV-Settings') throw '配置文件格式不正确'
...
async function exportConfig() {
  ...
  const settingsToExport = ['selectedAPIs', 'customAPIs', 'yellowFilterEnabled', 'adFilteringEnabled', 'doubanEnabled', 'hasInitializedDefaults']
  ...
  config['name'] = 'NasCoreTV-Settings' // 配置文件名，用于校验
  ...
  // 将配置数据保存为 JSON 文件
  saveStringAsFile(JSON.stringify(config), 'NasCoreTV-Settings_' + times + '.json')
}
