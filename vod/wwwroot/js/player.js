...
function initializePageContent() {
  ...
  // 设置页面标题
  document.title = currentVideoTitle + ' - NasCoreTV'
...
function initPlayer(videoUrl) {
  ...
  function hideControls() {
    ...
    if (art && art.controls && art.controls.classList) {
      // 添加检查
    ...
    try {
      ...
      if (historyRaw) {
        ...
        const idx = history.findIndex(
          (item) => item.title === currentVideoTitle && (item.episodeIndex === undefined || item.episodeIndex === currentEpisodeIndex)
        )
        if (idx !== -1) {
          // 只在进度有明显变化时才更新，减少写入
