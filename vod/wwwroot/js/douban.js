...
function fillAndSearch(title) {
  ...
  if (input) {
    ...
    try {
      ...
      window.history.pushState({ search: safeTitle }, `搜索: ${safeTitle} - NasCoreTV`, `./?s=${encodedQuery}`)
      // 更新页面标题
      document.title = `搜索: ${safeTitle} - NasCoreTV`
    } catch (e) {
...
async function fetchDoubanData(url) {
  ...
  try {
    console.log('DEBUG: Fetching proxied Douban:', PROXY_URL + encodeURIComponent(url)) // 调试信息
