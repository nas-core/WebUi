...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (path.startsWith(searchPrefix)) {
    ...
    if (keyword) {
      ...
      setTimeout(() => {
        ...
        try {
          window.history.replaceState({ search: keyword }, `搜索: ${keyword} - NasCoreTV`, window.location.href)
        } catch (e) {
  ...
  if (searchQuery) {
    ...
    setTimeout(() => {
      ...
      // 更新URL为规范格式
      try {
        window.history.replaceState({ search: searchQuery }, `搜索: ${searchQuery} - NasCoreTV`, `./?s=${encodeURIComponent(searchQuery)}`)
      } catch (e) {
        console.error('更新浏览器历史失败:', e)
      }
    }, 300)
