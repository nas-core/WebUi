...
function saveSearchHistory(query) {
  ...
  history = history.filter((item) => typeof item === 'object' && item.timestamp && now - item.timestamp < 5184000000)

  // 删除已存在的相同项
  history = history.filter((item) => (typeof item === 'object' ? item.text !== query : item !== query))
...
async function playFromHistory(url, title, episodeIndex, playbackPosition = 0) {
  ...
  try {
    ...
    // 构造播放器URL
    let playerUrl
    const sourceNameForUrl = historyItem
      ? historyItem.sourceName
      : new URLSearchParams(new URL(url, window.location.origin + window.location.pathname).search).get('source')
    const sourceCodeForUrl = historyItem
      ? historyItem.sourceCode || historyItem.sourceName
      : new URLSearchParams(new URL(url, window.location.origin + window.location.pathname).search).get('source_code')
    ...
    if (url.includes('player.html') || url.includes('watch.html')) {
      ...
      try {
        const nestedUrl = new URL(url, window.location.origin + window.location.pathname)
      ...
      const playUrl = new URL(url, window.location.origin + window.location.pathname)
...
function addToViewingHistory(videoInfo) {
  try {
    ...
    const existingIndex = history.findIndex(
      (item) => item.title === videoInfo.title && item.sourceName === videoInfo.sourceName && item.showIdentifier === videoInfo.showIdentifier // Strict check using the determined showIdentifier
    )
